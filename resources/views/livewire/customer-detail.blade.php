...
<div>
    @section('title', __('Customer Score Detail:') . ' ' . $customer->name)

    <div class="mb-6">
        <a href="{{ route('customers.index') }}"
            class="text-sm text-slate-500 hover:text-primary transition flex items-center gap-1">
            <span class="material-symbols-outlined text-sm">arrow_back</span> {{ __('Back to Customers') }}
        </a>
    </div>
    ...
    @if(!$score)
        <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-xl p-6 shadow-sm">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h3 class="font-bold text-lg">{{ __('No Score Available') }}</h3>
            </div>
            <p>{{ __("This customer hasn't been scored yet. Please recalculate scoring from the dashboard.") }}</p>
        </div>
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div
                ...
                <div class="w-full pt-6 border-t border-slate-100">
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{{ __('Total Score') }}</p>
            ...
            <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                <h3 class="text-lg font-bold mb-6">{{ __('Score Breakdown (1-5 Scale)') }}</h3>

                <div class="space-y-6">
                    <div class="grid grid-cols-12 gap-4 items-center">
                        <div class="col-span-4 font-medium text-slate-600">{{ __('Profitability') }}</div>
                    ...
                    <div class="grid grid-cols-12 gap-4 items-center">
                        <div class="col-span-4 font-medium text-slate-600">{{ __('Effort (Inversed)') }}</div>
                        <div class="col-span-6 bg-slate-100 h-2 rounded-full overflow-hidden">
                    ...
                    <div class="grid grid-cols-12 gap-4 items-center">
                        <div class="col-span-4 font-medium text-slate-600">{{ __('Chemistry') }}</div>
                        <div class="col-span-6 bg-slate-100 h-2 rounded-full overflow-hidden">
                    ...
                    <div class="grid grid-cols-12 gap-4 items-center">
                        <div class="col-span-4 font-medium text-slate-600">{{ __('Growth Potential') }}</div>
                        <div class="col-span-6 bg-slate-100 h-2 rounded-full overflow-hidden">
                    ...
                    <div class="grid grid-cols-12 gap-4 items-center">
                        <div class="col-span-4 font-medium text-slate-600">{{ __('Repeat Rate') }}</div>
                        <div class="col-span-6 bg-slate-100 h-2 rounded-full overflow-hidden">
            ...
            <div class="lg:col-span-3 bg-slate-50 border border-slate-200 rounded-xl p-6 mt-4">
                <h4 class="font-bold text-sm text-slate-500 uppercase tracking-wider mb-4">{{ __('Raw Input Data') }}</h4>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                        <span class="block text-slate-400">{{ __('Revenue') }}</span>
                        <span class="font-medium">€{{ number_format($customer->revenue, 2) }}</span>
                    </div>
                    <div>
                        <span class="block text-slate-400">{{ __('Margin EUR') }}</span>
                        <span class="font-medium">€{{ number_format($customer->profit_margin_eur, 2) }}</span>
                    </div>
                    <div>
                        <span class="block text-slate-400">{{ __('Effort Hours') }}</span>
                        <span class="font-medium">{{ $customer->effort_hours }}</span>
                    </div>
                    <div>
                        <span class="block text-slate-400">{{ __('Margin / Hour') }}</span>
                        <span
                            class="font-medium font-bold text-primary">€{{ number_format($score->margin_per_hour, 2) }}/{{ __('hr') }}</span>
                    </div>
                </div>
            </div>
        </div>
